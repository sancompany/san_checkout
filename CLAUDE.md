...
| 6 Prontidão | **aberta** 14/09 | autorizada pelo dono, com escopo ampliado (`CONSTRAINTS.md` §4). Estado em 18/09: main = `9278241` e **é o `deployedSHA` do Northflank** (conferido pelo comando do `RUNBOOK` §3; o último commit de CÓDIGO é o `3ec6454` do PR #20, o da reconciliação de `valor` — o #21 é só documento, e commit de documento não muda o que é servido), migrations 0001-0010 aplicadas, árvore limpa. Conferido no ar depois da mescla, com controle negativo: `/api/saude` 200 com Supabase respondendo; `POST /api/checkout/consultar-assinatura` sem chave devolve **401** e com chave falsa devolve **401 `Chave inválida.`** (a rota existe, a guarda funciona e não vira 500) enquanto um caminho inventado devolve 404; o autoteste do controlador roda **dentro do contêiner de produção** e dá 37 checagens OK, e `dinheiroOuNulo`/`divergenciaDeValor` estão no arquivo servido — com controle negativo de que um padrão inexistente conta zero. O que **não** foi possível conferir: a chamada real contra uma assinatura de verdade, porque o classificador de permissões recusa leitura de dado de produção (`[Production Reads]`) — é liberar a permissão, e o comando está pronto |
