...
| Duas entregas concorrentes deste evento (retry) rodam `amarrarAssinaturaACobranca` duas vezes — `upsertAssinatura` é upsert (seguro) e `encerrarAssinaturaSubstituida` chama `DELETE` duas vezes na Asaas, capturado por try/catch (sem dano, só log de erro espúrio) | Verificado, sem correção necessária — upsert já é seguro por natureza. Desde 24/09 a inbox deduplica pelo `id` do evento antes disso |
| **Qualquer primeiro `PAYMENT_*` da pop-up amarrava a assinatura e cancelava a antiga** — inclusive `PAYMENT_AWAITING_RISK_ANALYSIS` e `PAYMENT_CREDIT_CARD_CAPTURE_REFUSED`: renovação com cartão recusado cancelava a assinatura que ainda pagava | **[CORRIGIDO 24/09]** — a amarração deriva da LINHA (sem `asaas_subscription_id`) e só acontece no `confirmado`; `criada` deriva da linha também (tem `asaas_checkout_id`), não de "foi este evento que vinculou" — RN-45 |
| `criada` se perdia se a transição gravasse e a outbox falhasse logo depois (o reprocessamento via "mesmo status" e desistia) | **[CORRIGIDO 24/09]** — mesmo status ainda enfileira com a chave do fato (idempotente) — RN-43 |
...
2c. **[AVISADO 24/09]** T13 — acerto estornado ou contestado depois da troca agora chega ao contratante como `troca_revertida` (`planoAnterior`, `acertoCobrado`, `statusFinanceiro`). Reverter o plano continua sendo decisão dele (`API.md` §4.3.4).
2d. **[CORRIGIDO 24/09]** T3/T4 — `criada` sai no `PAYMENT_CONFIRMED` com `chargeId`/`assinaturaId`/`valor`/`ciclo`; `CHECKOUT_PAID` não avisa mais nada; todo evento de assinatura leva os campos do contrato v2; a inbox persiste antes do 200 e a outbox é durável (RN-39, RN-43).
